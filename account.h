...
public:
    Account (int ID,string ten_dang_nhap,string ten_tai_khoan, int gioi_tinh,my_time ngay_sinh,string email,int doi_tuong, string phone_number,string pass,string level,int score,my_time date_created){
        this->ID = ID;
    ...
    ~Account () {}
    bool is_admin() const{
        return doi_tuong == ADMIN;
    };
    bool is_librarian() const{
        return doi_tuong == LIBRARIAN;
    };
    bool is_reader() const{
        return doi_tuong == READER;
    };
    
    bool is_quyen_tri_vien() const{
        return is_admin() || is_librarian();
    };
    string chuc_vu() const{
        if (is_admin()) return "Quản trị viên";
        if (is_librarian()) return "Thủ thư";
        return "Người dùng";
    };
    void ma_hoa_();
    void giai_ma_();
